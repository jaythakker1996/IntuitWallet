...
### 2026-05-06 — Dev tooling: API docs + local DB inspection

- **API documentation: springdoc-openapi.** Adopted Springdoc-OpenAPI for REST API documentation. Adds `org.springdoc:springdoc-openapi-starter-webmvc-ui:2.6.0` (the line that supports Spring Boot 3.3.x). Swagger UI lives at `/swagger-ui/index.html`; the OpenAPI v3 JSON document lives at `/v3/api-docs`. Annotations are not required — the doc is generated from controller method signatures, validation annotations, and Jackson-visible record fields. Per-endpoint customization (`@Operation`, `@Tag`) is left to feature authors when the default isn't enough. A single `OpenApiConfig` bean in `service` carries title/version/description metadata.
- **Local DB inspection: pgweb.** Added a `pgweb` service (`sosedoff/pgweb:0.16.2`) to `docker-compose.yml`, exposed on host port `8082`, connected to `app-postgres` over the existing `wallet-net` network via `DATABASE_URL`. Dev-only; production DB inspection lives elsewhere and is out of scope for the POC.
- **Spec 003 removed.** The Update User spec was Draft and never implemented; per direction it has been removed entirely. Spec 002 (Create) and spec 004 (Get) remain. ADR 002 has been adjusted to drop the PATCH endpoint reference and to note that `updated_at` stays in the schema as forward-compatible audit signal until a future Update spec returns.
